...
export const Posts = () => {
  const { data, loading, fetchMore, error } = useQuery<IEquilibriumPostsData>(
  ...
  const loadMoreTopics = () => {
    if (loading || !data?.equilibriumPostsConnection.pageInfo.hasNextPage)
      return;
  ...
  const hasMore =
    data?.equilibriumPostsConnection.pageInfo.hasNextPage ?? false;
    ...
    <Container>
      <div className="content">
        {loading ? (
          ...
          <>
            <h2 className="postsTitle">Publicações sobre nossas terapias:</h2>
            <PostsList
              infoEdge={data.equilibriumPostsConnection.edges}
              hasMoreData={hasMore}
